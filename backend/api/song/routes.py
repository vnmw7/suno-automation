...
async def review_song_endpoint(request: SongReviewRequest):
    ...
    """
    try:
        print(f"[review_song_endpoint] Starting review for audio file: {request.audio_file_path}")
        ...
        if not request.song_structure:
            raise HTTPException(
                status_code=400,
                detail="Song structure is required"
            )
        ...
        print("[review_song_endpoint] Audio file validated, starting AI review process...")
        ...
        if not review_result["success"]:
            print(f"[review_song_endpoint] Review failed: {review_result.get('error', 'Unknown error')}")
        ...
        print(f"[review_song_endpoint] Review completed successfully with verdict: {review_result['verdict']}")
